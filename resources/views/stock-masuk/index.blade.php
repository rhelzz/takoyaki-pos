...
@section('title', 'Stock Masuk - Takoyaki POS')

@section('content')
<div class="min-h-screen bg-gray-50" x-data="stockMasuk()">
    <!-- Header - Mobile Optimized -->
    <div class="bg-white border-b border-gray-200 px-4 py-4 sticky top-0 z-10">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl md:text-2xl font-bold text-gray-800">Stock Masuk</h1>
                <p class="text-sm text-gray-600 hidden sm:block">Kelola barang masuk</p>
            </div>
            <!-- Mobile Quick Actions -->
            <div class="flex space-x-2">
                <button @click="showFilters = !showFilters" 
                        class="lg:hidden bg-blue-500 text-white p-2 rounded-lg">
                ...
                <div class="space-y-3 lg:space-y-0 lg:grid lg:grid-cols-4 lg:gap-4">
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex space-x-2 lg:pt-0">
                            <button type="submit" 
                                    class="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors" id="stock-row-{{ $stock->id }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-3">
                                            <a href="{{ route('stock-masuk.show', $stock) }}" 
                                               class="text-blue-600 hover:text-blue-800 text-sm">
                                                <i class="fas fa-eye mr-1"></i>Detail
                                            </a>
                                            <a href="{{ route('stock-masuk.edit', $stock) }}" 
                            ...
                            <div class="p-4 bg-gray-50 border-b border-gray-100">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-right">
                                        <div class="font-bold text-blue-600">+{{ $stock->qty }}</div>
                                        <div class="text-xs text-gray-500">Stock Masuk</div>
                                    </div>
                                </div>
                            </div>
            ...
            @else
                ...
                <div class="p-8 text-center">
                    <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <i class="fas fa-arrow-up text-2xl text-gray-400"></i>
                    </div>
                    <h3 class="text-lg font-medium text-gray-800 mb-2">Belum ada stock masuk</h3>
                    <p class="text-gray-500 mb-6">Mulai catat barang yang masuk</p>
                    <a href="{{ route('stock-masuk.create') }}" 
                       class="inline-flex items-center px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium">
                        <i class="fas fa-plus mr-2"></i>Tambah Stock Masuk
                    </a>
                </div>
            @endif
        </div>
...
function stockMasuk() {
    return {
        ...
        deleteStock(id, name) {
            if (confirm(`Apakah Anda yakin ingin menghapus stock masuk "${name}"?`)) {
                fetch(`/stock-masuk/${id}`, {
                    method: 'DELETE',
                    headers: {
