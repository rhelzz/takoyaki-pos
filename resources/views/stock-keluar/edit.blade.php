...
@section('title', 'Edit Stock Keluar - Takoyaki POS')

@section('content')
<div class="min-h-screen bg-gray-50" x-data="stockFormEdit()">
    <div class="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div class="px-4 py-4">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-lg font-bold text-gray-800">Edit Stock Keluar</h1>
                    <p class="text-sm text-gray-500">{{ $stockKeluar->judul }}</p>
                </div>
            </div>
        </div>
    </div>
    <div class="p-4 max-w-2xl mx-auto">
        <form action="{{ route('stock-keluar.update', $stockKeluar) }}" method="POST" class="space-y-4" @submit="prepareSubmit">
            @csrf
            @method('PUT')
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
                <h3 class="text-lg font-semibold text-gray-800">Edit Data Stock Keluar</h3>
                <div>
                    <label for="judul" class="block text-sm font-medium text-gray-700 mb-2">
                        Judul <span class="text-red-500">*</span>
                    </label>
                    <input type="text" id="judul" name="judul" value="{{ old('judul', $stockKeluar->judul) }}" required
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi"
                              class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                              placeholder="Deskripsi laporan">{{ old('deskripsi', $stockKeluar->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="date" id="tanggal" name="tanggal"
                           value="{{ old('tanggal', $stockKeluar->tanggal->format('Y-m-d')) }}"
                           required
                           class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    @error('tanggal')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-3">
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i>Update Stock Keluar
                </button>
...
function stockFormEdit() {
    return {
        toppings: @json($stockKeluar->toppings ?? []),
        packagings: @json($stockKeluar->packagings ?? []),
        toppingsJson: '',
        packagingsJson: '',
