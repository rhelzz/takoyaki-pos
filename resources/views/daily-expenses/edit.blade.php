...
@section('title', 'Edit Pengeluaran Harian - Takoyaki POS')
...
<div class="min-h-screen bg-gray-50" x-data="expenseForm()">
    ...
    <div class="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div class="px-4 py-4">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-lg font-bold text-gray-800">Edit Pengeluaran</h1>
                    <p class="text-sm text-gray-500">{{ $dailyExpense->nama_pengeluaran }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="p-4 max-w-2xl mx-auto">
        <!-- Form -->
        <form action="{{ route('daily-expenses.update', $dailyExpense) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('nama_pengeluaran', $dailyExpense->nama_pengeluaran) }}"
                           required
                ...
                <div>
                    ...
                    <input type="date" 
                           id="tanggal" 
                           name="tanggal" 
                           value="{{ old('tanggal', $dailyExpense->tanggal->format('Y-m-d')) }}"
                           required
                           class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
                    @error('tanggal')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" 
                              ...
                              rows="3"
                              class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                              placeholder="Deskripsi tambahan...">{{ old('deskripsi', $dailyExpense->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
function expenseForm() {
    return {
        items: {!! json_encode($dailyExpense->items->map(function($item) {
            return [
